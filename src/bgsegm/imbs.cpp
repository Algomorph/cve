...
/**
 * Pixels satisfying all of the following shadow suppression constraints will be labeled as shadow and excluded from FG:
 * alpha <= [value ratio between image and background model pixel] < beta
 * [saturation difference between image and background model pixel] <= tau_s
 * [hue difference between image and background model pixel] <= tau_h
 * @param alpha lower inclusive bound on value ratio between frame pixel & background model pixel to be considered shadow
 * @param beta upper exclusive bound on value ratio between frame pixel & background model pixel to be considered shadow
 * @param tau_s upper inclusive bound on saturation difference between frame & background model pixel
 *                       to be considered shadow
 * @param tau_h upper inclusive bound on hue difference between frame & background model pixel to be considered shadow
 * @param sampling_interval duration of the sampling period, in milliseconds
 * @param use_morphological_filtering apply morphological opening and closing or not
 */
    BackgroundSubtractorIMBS::BackgroundSubtractorIMBS(
            ...
            double tau_h,
            double minArea,
            double persistence_period,
            bool use_morphological_filtering,
            bool rebuild_model) :
            fps(fps),

            alpha(alpha),
            beta(beta),
            tau_saturation(tau_s),
            tau_hue(tau_h),
            ...
            minArea(minArea),
            persistence_period(persistence_period),
            fgThreshold(fgThreshold),

            morphological_filtering(use_morphological_filtering),
    ...
    /**
     * First time initialization that allocates all the necessary arrays using the provided frame size & type
     */
    ...
    cv::Mat BackgroundSubtractorIMBS::apply_pythonic(cv::Mat image) {
        cv::Mat mask;
        this->apply(image, mask);
        return mask;
    }
    ...
    void BackgroundSubtractorIMBS::apply(InputArray _frame, OutputArray _fgmask, double learningRate) {
        ...
        if (fps == 0.) {
            timestamp = getTimestamp();//ms of execution time
        } else {
            //advance to next frame time
            timestamp += 1000. / fps;//ms of video
        }
    ...
    void BackgroundSubtractorIMBS::updateBg() {
        ...
        if (bg_frame_counter == numSamples - 1) {
            //TODO are we missing 'frame.copyTo(bgSample);' here?
            createBg(bg_frame_counter); // max number of samples to construct a background is reached
            // clear off background frame counter to start to accumulate samples for an updated model
            if(this->rebuild_model){
                bg_frame_counter = 0;
            }else{
                bg_frame_counter = 9102;// TODO: super-horrible hacky solution
            }
        } else if (bg_frame_counter != 9102 && (timestamp - prev_bg_frame_time) >= sampling_interval) {
            //bg_frame_counter < (numSamples - 1)
            //if time elapsed since last frame was sampled is at or greater than the sampling interval,
            //get a new sample for the bg model
            prev_bg_frame_time = timestamp;
    ...
    void BackgroundSubtractorIMBS::hsvSuppression() {
        ...
        uchar image_hue, image_saturation, image_value;
        uchar background_hue, background_saturation, background_value;
        float hue_diff, saturation_diff, value_ratio;
        ...
        for (unsigned int p = 0; p < numPixels; ++p) {
            if (fgmask.data[p]) {
                ...
                image_hue = imHSV[0].data[p];
                image_saturation = imHSV[1].data[p];
                image_value = imHSV[2].data[p];
                ...
                for (unsigned int n = 0; n < maxBgBins; ++n) {
                    ...
                    background_hue = hsvPixel.at<cv::Vec3b>(0, 0)[0];
                    background_saturation = hsvPixel.at<cv::Vec3b>(0, 0)[1];
                    background_value = hsvPixel.at<cv::Vec3b>(0, 0)[2];

                    value_ratio = (float) image_value / (float) background_value;
                    saturation_diff = std::abs(image_saturation - background_saturation);
                    hue_diff = std::min(std::abs(image_hue - background_hue),
                                        255 - std::abs(image_hue - background_hue));

                    if (hue_diff <= tau_hue &&
                        saturation_diff <= tau_saturation &&
                        value_ratio >= alpha &&
                        value_ratio < beta) {
    ...
    void BackgroundSubtractorIMBS::createBg(unsigned int bg_sample_number) {
        ...
        for (unsigned int p = 0; p < numPixels; ++p) {
            ...
            if (bg_sample_number == 0) {
                ...
                for (unsigned int s = 0; s < bg_sample_number; ++s) {
                    ...
                    if ((unsigned) std::abs(currentPixel[2] - bgBins[p].binValues[s][2]) <= associationThreshold &&
                        (unsigned) std::abs(currentPixel[1] - bgBins[p].binValues[s][1]) <= associationThreshold &&
                        (unsigned) std::abs(currentPixel[0] - bgBins[p].binValues[s][0]) <= associationThreshold) {
                        ...
                        if (index == maxBgBins) {
                            ...
                            if (fgmask.data[p] == PERSISTENCE_LABEL) {
                                for (unsigned int n = 0; n < maxBgBins; n++) {
                                    ...
                                    unsigned int d = std::max(
                                            (int) std::abs(bgModel[p].values[n][0] - bgBins[p].binValues[s][0]),
                                            std::abs(bgModel[p].values[n][1] - bgBins[p].binValues[s][1]));
                                    d = std::max((int) d,
                                                 std::abs(bgModel[p].values[n][2] - bgBins[p].binValues[s][2]));
                                    if (d < fgThreshold) {
        ...
        //new background model is finished
        if (bg_sample_number == (numSamples - 1)) {
        ...
        for (unsigned int p = 0; p < numPixels; ++p) {
            ...
            if (isFg) {
                if (conditionalUpdated) {
                    ...
                    if (persistenceMap[p] > persistence_period) {
                        for (unsigned int n = 0; n < maxBgBins; ++n) {
                            if (!bgModel[p].isValid[n]) {
                                break;
                            }
